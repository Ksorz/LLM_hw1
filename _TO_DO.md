...
* POST /evaluate \- в котором необходимо реализовать параметры /forward\_batch, но дополнительно ещё необходимо вернуть метрики для загруженного датасета. Продумайте удобный формат для загрузки данных:  
  * Реализовано: perplexity/avg_loss для текстов без таргетов (JSON/CSV) в PyTorch-режиме  
  * TODO: метрики по референсам (ROUGE/BLEU/acc) и поддержка медиа (.zip) при необходимости
...
* POST /deploy/\<int:experiment\_id\> \- заменяет текущую модель эксперимента на новую с необходимым номером эксперимента  
  * Статус: базовый вариант реализован (CSV text/label, subprocess retrain, статус/пути в БД, hot-reload deploy). Расширения: метрики/референсы/медиа по необходимости.
